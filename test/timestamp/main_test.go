...
func TestStdout(t *testing.T) {
	...
	checkStream(t, r.stdout, "echo hello", "hello")
...
func TestQuotedCommand(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			r := runCmd(t, cmd)
			checkExit(t, r.exit, 0)
			checkStream(t, r.stdout, tc.want)
			checkStream(t, r.stderr)
		})
	}
}
...
func TestStderr(t *testing.T) {
	...
	checkExit(t, r.exit, 0)
	checkStream(t, r.stdout, `sh -c "echo err >&2"`)
...
func TestStdoutAndStderr(t *testing.T) {
	...
	checkExit(t, r.exit, 0)
	checkStream(t, r.stdout, `sh -c "echo out; echo err >&2"`, "out")
	checkStream(t, r.stderr, "err")
}
...
func TestStdin(t *testing.T) {
	...
	r := runCmd(t, cmd)
	checkExit(t, r.exit, 0)
	checkStream(t, r.stdout, "cat", "hello from stdin")
	checkStream(t, r.stderr)
}
