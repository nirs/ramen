...
def exists(profile):
    names = _run(LIMACTL, "list", "--format", "{{.Name}}", context=profile["name"])
...
def start(profile, verbose=False):
    ...
    if not exists(profile):
        ...
        with tempfile.NamedTemporaryFile(
            ...
            _create_cluster(profile, tmp.name)

    _start_cluster(profile)
    cluster = _get_cluster(profile)
    _add_kubeconfig(profile, cluster)
...
def _get_cluster(profile):
    out = _run(LIMACTL, "list", "--format", "json", context=profile["name"])
    for line in out.splitlines():
        cluster = json.loads(line)
        if cluster["name"] == profile["name"]:
            return cluster
    return None
...
def _add_kubeconfig(profile, cluster):
    logging.debug("[%s] Adding lima cluster kubeconfig", profile["name"])
    src = os.path.join(cluster["dir"], "copied-from-guest", "kubeconfig.yaml")
...
def _create_cluster(profile, config):
    _watch(
        LIMACTL,
        "create",
        "--name",
...
def _stop_cluster(profile):
    _watch(LIMACTL, "stop", profile["name"], context=profile["name"])


def _delete_cluster(profile):
    # --force allows deletion of a running cluster.
    _watch(LIMACTL, "delete", "--force", profile["name"], context=profile["name"])
...
def _delete_disk(profile, disk):
    _watch(LIMACTL, "disk", "delete", disk["name"], context=profile["name"])
